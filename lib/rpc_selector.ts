...
export class RPCSelector implements RPCSelectorInterface {
    ...
    private async callSelectedClient<T>(methodName: string, args: any[]): Promise<T> {
        logger.info("RPCSelector: Calling existing client", {
            clientName: this.selectedClient?.getURL().toString() || 'unknown',
            methodName,
            arguments: args
        })
	...
	private async callFallbackClients<T>(methodName: string, args: any[]): Promise<T> {
        ...
        try {
            const { result, client } = await new Promise<{ result: T; client: WrappedSuiClient }>((resolve, reject) => {
                let resolved = false;
                let rejectedCount = 0;
                const errors: any[] = [];

                clientPromises.forEach((promise) => {
                    promise.then((value) => {
                        if (!resolved) {
                            resolved = true;
                            resolve(value);
                        ...
                        errors.push(error);
                        rejectedCount++;
                        if (rejectedCount === clientPromises.length) {
                            reject(errors);
                        }
                    });
                });
