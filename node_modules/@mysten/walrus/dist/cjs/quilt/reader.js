...
class QuiltReader {
  ...
  async readIndex() {
    ...
    const index = import_bcs2.QuiltIndexV1.parse(indexBytes);
    return index.patches.map((patch, i) => {
      ...
      return {
        ...
        patchId: (0, import_utils.urlSafeBase64)(
          import_bcs2.QuiltPatchId.serialize({
            quiltId: this.blobId,
            patchId: {
              version: 1,
              startIndex,
...
readBlob_fn = async function(sliverIndexes) {
  ...
  const blobHeader = import_bcs2.QuiltPatchBlobHeader.parse(firstSliver);
  ...
  offset += 2;
  const identifier = import_bcs.bcs.string().parse(firstSliver.subarray(offset, offset + identifierLength));
  ...
  if (blobHeader.mask & import_quilts.HAS_TAGS_FLAG) {
    ...
    offset += 2;
    tags = import_bcs2.QuiltPatchTags.parse(firstSliver.subarray(offset, offset + tagsSize));
    blobSize -= tagsSize + 2;
    offset += tagsSize;
  }
...
getSizes_fn = function(encodedBlobSize) {
  const { primarySymbols, secondarySymbols } = (0, import_utils.getSourceSymbols)(__privateGet(this, _numShards));
